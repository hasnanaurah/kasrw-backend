...
def create(data: TransaksiBase):
    ...
    cursor = db.cursor()
    cursor.execute(
        "INSERT INTO transaksi (tanggal, keterangan, jenis, jumlah) VALUES (%s,%s, %s, %s)", (data.tanggal, data.keterangan, data.jenis, data.jumlah),
        )
    db.commit()
...
def update(id: int, data: TransaksiUpdate):
    ...
    if not row:
        ...
        cursor.execute(
            "UPDATE transaksi SET tanggal=%s, keterangan=%s, jenis=%s, jumlah=%s WHERE id=%s", (updated["tanggal"], updated["keterangan"], updated["jenis"], updated["jumlah"], id),
            )
    db.commit()
    db.close()
